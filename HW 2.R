...
#LM function
airbnb.lm.train <- lm(reviews_per_month ~ price, data=train)
airbnb.lm.test <- lm(reviews_per_month ~ price, data=test)

#Linear regression on training data
smooth <- ggplot(data=train, aes(x=price, y=reviews_per_month, color=room_type)) + 
...
smooth <- ggplot(data=test, aes(x=price, y=reviews_per_month, color=room_type)) + 
  geom_point(aes(shape=room_type), size=1.5) + xlab("Price") + ylab("Reviews Per Month") + 
